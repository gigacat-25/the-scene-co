...
export default function AdminTeamPage() {
  ...
  const [form, setForm] = useState({ name: "", role: "", bio: "", image_url: "", order_index: 0, is_published: 1 });
  ...
  function resetForm() {
    setForm({ name: "", role: "", bio: "", image_url: "", order_index: members.length, is_published: 1 });
    setEditingId(null);
    setShowForm(false);
  }
  ...
  function handleEdit(member: any) {
    ...
    setForm({
      ...
      is_published: member.is_published ?? 1,
    });
    setShowForm(true);
  ...
  async function togglePublish(member: any) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-ink font-bold" style={{ fontSize: 32, fontWeight: 540 }}>Team</h1>
        <button
          onClick={() => { setShowForm(!showForm); if (showForm) resetForm(); }}
          className="btn-primary-figma text-sm px-4 py-2 flex items-center"
        >
          <Plus className="h-4 w-4 mr-2" /> {showForm ? "Cancel" : "Add Member"}
        </button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="bg-canvas border border-hairline rounded-lg p-6 mb-8 shadow-sm">
          <h2 className="text-ink font-bold text-lg mb-5">{editingId ? "Edit" : "New"} Team Member</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="caption-mono text-ink/50 text-xs">Name *</label>
                  <input
                    placeholder="e.g. Thejaswin P S"
                    value={form.name}
                ...
                <div className="space-y-1.5">
                  <label className="caption-mono text-ink/50 text-xs">Role *</label>
                  <input
              ...
              <div className="space-y-1.5">
                <label className="caption-mono text-ink/50 text-xs">Bio</label>
                <textarea
                  placeholder="Short bio — shown below the member's photo on the website..."
                  value={form.bio}
                  onChange={e => setForm({ ...form, bio: e.target.value })}
                  className="w-full h-28 bg-surface-soft border border-hairline rounded-md px-3 py-2 text-sm text-ink outline-none focus:border-ink resize-none"
                />
              </div>
            ...
            <div className="space-y-2">
              <label className="caption-mono text-ink/60 text-xs">Photo</label>
          ...
          <div className="flex justify-end gap-3 pt-5 border-t border-hairline mt-5">
            ...
            <button type="submit" disabled={submitting} className="btn-primary-figma text-sm px-6 py-2 flex items-center">
              {submitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : "Save Member"}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="space-y-4">
        ...
        {members.map(member => (
          <div
            key={member.id}
            className={`bg-canvas border rounded-lg p-5 flex items-center gap-5 hover:border-ink/20 transition-colors ${member.is_published ? "border-hairline" : "border-dashed border-hairline opacity-60"}`}
          >
            ...
            {member.image_url ? (
              <img
                ...
                alt={member.name}
                className="w-14 h-14 object-cover rounded-full border border-hairline shrink-0"
              />
            ) : (
              <div className="w-14 h-14 rounded-full bg-surface-soft border border-hairline shrink-0 flex items-center justify-center text-ink/30 text-xl font-bold">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-ink font-semibold text-sm">{member.name}</p>
                {!member.is_published && (
                  <span className="caption-mono text-[9px] bg-ink/10 text-ink/50 px-1.5 py-0.5 rounded">HIDDEN</span>
                )}
              </div>
              <p className="caption-mono text-ink/50 text-[10px] mt-0.5">{member.role}</p>
              {member.bio && (
                <p className="text-ink/60 text-xs mt-1 truncate max-w-lg">{member.bio}</p>
              )}
            </div>
